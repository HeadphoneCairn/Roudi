...
PageNameChannel::PageNameChannel(/*uint8_t channel_value*/): Page(), m_channel_value(0), m_position(0)
{
}

void PageNameChannel::OnStart()
{
  m_channel_value = 4; m_position = 0;
  SetNumberOfLines(7, 1, 0);
  sprintf(Screen::buffer + MaxNameLength, GetPString(PSTR_layout_channel), m_channel_value + 1);
  m_name.Init(Screen::buffer, GetChannelNameBrol(m_channel_value));
}
...
Page::LineResult PageNameChannel::Line(LineFunction func, uint8_t line, uint8_t field)
{
...
uint8_t PageNameChannel::GetMaxPositions()
{
  return m_name.GetPositions() + 3;
}

uint8_t PageNameChannel::GetPositionOfChannel()
{
  return m_name.GetPositions();
}

uint8_t PageNameChannel::GetPositionOfAccept()
{
  return m_name.GetPositions() + 1;
}

uint8_t PageNameChannel::GetPositionOfCancel()
{
